...
```html
<!-- svelte/src/components/Waves.svelte -->
...
cd svelte-vite-web3
yarn create vite svelte --template svelte
mkdir components hardhat hardhat/contracts hardhat/scripts
echo > svelte/components/Waves.svelte
...
echo '.env' >> .gitignore
cd svelte
yarn add -D ethers dayjs vercel
cd ..
cd hardhat
yarn init -y
